...
export default function MatchesPage() {
  ...
  if (!project) {
    return <p className="text-sm text-zinc-400">Puzzle introuvable.</p>;
  }
  ...
  return (
    <>
      ...
      <div className="mb-4 flex gap-2">
        <PrimaryButton
          variant="ghost"
          onClick={() => update(store.undoLast(project))}
        >
          Annuler
        </PrimaryButton>
        <PrimaryButton
          variant="ghost"
          onClick={() => update(store.recomputeMatches(project))}
        >
          Recalculer
        </PrimaryButton>
      </div>
          ...
          >
            <div>
              ...
              <div className="text-[11px] text-zinc-500">
                {m.sideA} ↔ {m.sideB} · conf. {(m.confidence * 100).toFixed(0)}%
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <PrimaryButton
                variant="success"
                onClick={() => {
                  update(store.confirmMatch(project, selected.id));
                  setSelected(null);
                }}
              >
                Confirmer
              ...
              <PrimaryButton
                variant="danger"
                onClick={() => {
                  update(store.rejectMatch(project, selected.id));
                  setSelected(null);
                }}
              >
                Rejeter
